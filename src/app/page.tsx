...
import CollectionList from '@/components/CollectionList'; // <-- Красивые слайдеры
...
export default async function Home() {
  ...
  try {
    ...
    const [
      ...
      getMediaCollection('movie', 'now_playing'),  // Сейчас в кино
      getMediaCollection('movie', 'upcoming'),     // Скоро
      getMediaCollection('tv', 'popular'),         // Тренды сериалов
      getMediaCollection('movie', 'top_rated'),    // Легенды
      fetchMoreMovies(1),                          // Для нижней сетки (все фильмы)
      fetchMoreTVShows(1)                          // Для нижней сетки (все сериалы)
    ]);
    ...
    // 2. Выбираем фильм для HERO (Главный баннер)
    // Берем первый фильм из "Сейчас в кино"
    ...
    // 3. Форматируем данные для нижних сеток (бесконечный скролл)
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-pink-500/30 selection:text-pink-100 overflow-x-hidden">
      ...
      <main className="relative z-10">
        ...
        {/* === ПОДБОРКИ (СЛАЙДЕРЫ) === */}
        {/* 1. Новинки (Яркий красный градиент) */}
        <CollectionList
            title="🍿 Сейчас в кино"
